...
def generate_daily_markdown(entry_date: date, data_dir: str, config: Dict[str, Any], output_dir: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"title: Azure News - {entry_date.isoformat()}\n")
        f.write(f"news_counter: {total_entries}\n")
        f.write(f"last_updated: {last_updated}\n")
        f.write("---\n\n")

        f.write(f"# Azure Updates - {entry_date.isoformat()}\n\n")
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description='Azure Feed購読システム')
