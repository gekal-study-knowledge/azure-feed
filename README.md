...
aws-feed とは**別の Firebase プロジェクト**が必要です。既読データのキー (`postId`) が
`YYYY/MM/DD/YYYY-MM-DD-news` 形式で両サイト共通のため、同じプロジェクトを共有すると
AWS 版と Azure 版の既読が混ざります。

1. Firebase コンソールで `azure-feed` プロジェクトを作成
...
6. `firebase deploy --only firestore:rules --project azure-feed` でルールを反映
