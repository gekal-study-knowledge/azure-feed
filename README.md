...
Azure / Microsoft 公式の RSS フィードを自動的に購読し、日単位でまとめた Markdown レポートを生成し、Next.js で閲覧できるシステムです。

## 機能

- **自動フィード取得**: Azure Updates、Azure Blog、Microsoft Security Blog など 9 つのフィードを取得
...
```bash
git clone https://github.com/gekal-study-knowledge/azure-feed.git
cd azure-feed
```
...
- **自動実行**: 1 時間ごと (毎時 0 分)
- **手動実行**: GitHub の Actions タブから「Fetch Azure Feeds」ワークフローを選択し、「Run workflow」をクリック
...
```
https://gekal-study-knowledge.github.io/azure-feed/
```
...
feeds:
  - name: 'Azure Updates'
    ...
    source_id: 'azure_updates'

  - name: 'Azure Blog'
    url: 'https://azure.microsoft.com/en-us/blog/feed/'
    source_id: 'azure_blog'

  - name: 'Microsoft Security Blog'
    url: 'https://www.microsoft.com/en-us/security/blog/feed/'
    source_id: 'microsoft_security_blog'

  - name: 'Azure Status'
    url: 'https://azure.status.microsoft/en-us/status/feed/'
    source_id: 'azure_status'

  - name: 'Azure Architecture Blog'
    url: 'https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=AzureArchitectureBlog'
    source_id: 'azure_architecture_blog'
  ...
  - name: 'Azure Infrastructure Blog'
    ...
    source_id: 'azure_infrastructure_blog'

  - name: 'Azure Networking Blog'
    url: 'https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=AzureNetworkingBlog'
    source_id: 'azure_networking_blog'

  - name: 'Apps on Azure Blog'
    url: 'https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=AppsonAzureBlog'
    source_id: 'apps_on_azure_blog'

  - name: 'Azure SDK Blog'
    url: 'https://devblogs.microsoft.com/azure-sdk/feed/'
    source_id: 'azure_sdk_blog'
...
Azure 側は AWS と URL 体系が異なるため、以下の点に注意して選定しています（いずれも疎通確認済み）。

| 項目           | 内容                                                                                                                                                 |
| :------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------- |
| Azure Updates  | `azure.microsoft.com/en-us/updates/feed/` は現在 HTML を返し RSS ではない。`www.microsoft.com/releasecommunications/api/v2/azure/rss` が現行の配信元 |
| 日本語ブログ   | `azure.microsoft.com/ja-jp/blog/feed/` は英語版へリダイレクトされる。AWS Japan Blog に相当するフィードは存在しない                                   |
| Tech Community | `t5/s/gxcuf89792/rss/board?board.id=<ID>` 形式。board.id が誤っていても HTTP 200 で `Resource Not Found` が返るため、追加時は必ず件数を確認する      |
| Azure Status   | 障害が無い間はエントリー 0 件。これは正常                                                                                                            |

MSRC の Security Update Guide（`https://api.msrc.microsoft.com/update-guide/rss`）は AWS Security
Bulletins に相当しますが、Azure に限らず Microsoft 製品全体の CVE を約 4,700 件・2.3MB で配信する
ため既定では含めていません。必要なら以下の JSON API で差分取得する方が現実的です。

- `https://api.msrc.microsoft.com/cvrf/v3.0/updates` — 月次リリース一覧
- `https://api.msrc.microsoft.com/sug/v2.0/en-US/vulnerability?$top=50&$orderby=releaseDate desc` — OData で件数・並び順を制御

その他の利用可能な board.id: `AzureGovernanceandManagementBlog` / `AzureStorageBlog` /
`MicrosoftDefenderCloudBlog` / `FinOpsBlog` / `AppsonAzureBlog` / `ITOpsTalkBlog` / `AzureArcBlog` / `AzureSQL`

devblogs 系: `https://devblogs.microsoft.com/{azure-sdk,devops,cosmosdb,identity,azuregov}/feed/`
...
```bash
# 2026年4月24日のエントリーを取得
curl https://gekal-study-knowledge.github.io/azure-feed/api/entries/2026/04/24/index.json
```
...
| `sourceId`   | `string` | 情報源の識別子 (例: `azure_updates`)               |
| `sourceName` | `string` | 情報源の人間が読みやすい名前 (例: `Azure Updates`) |

**利用例**:

```bash
curl https://gekal-study-knowledge.github.io/azure-feed/api/entries/all/index.json
```
...
**例**: `data/2026/03/2026-03-20/azure_updates.yaml`
...
---
layout: default
title: Azure News - 2026-03-20
news_counter: 15
last_updated: '2026-03-21 10:45:50 JST'
---

# Azure Updates - 2026-03-20
...
- **Firebase プロジェクト**: `azure-feed`（aws-feed とは別プロジェクト）
...
```bash
firebase deploy --only firestore:rules --project azure-feed
```
...
- [x] Firebase プロジェクト `azure-feed` の作成
...
curl -X PATCH -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"authorizedDomains":["localhost","azure-feed.firebaseapp.com","azure-feed.web.app","azure.news.gekal.cn","gekal-study-knowledge.github.io"]}' \
  "https://identitytoolkit.googleapis.com/admin/v2/projects/azure-feed/config?updateMask=authorizedDomains"
```
